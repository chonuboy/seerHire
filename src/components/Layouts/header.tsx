...
export default function SeerTechLogo() {
	...
	return (
		...
		>
			<button id="menu-toggle" className="md:hidden text-gray-700">
				<svg
			...
			<div className='flex items-center gap-2' onClick={() => setTimeout(() => router.push('/settings'),1000)}>
			{<h3 className="text-white font-semibold">{user?.replace('"','').replace('"','').charAt(0).toUpperCase()}{user?.slice(2,user.length-1)}</h3>}
				<img
